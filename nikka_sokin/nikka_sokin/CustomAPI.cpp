...
DWORD64 dwGetModuleBaseAddress(DWORD dwProcID, const wchar_t* modName)
{
	...
	if (hSnapshot != INVALID_HANDLE_VALUE)
	{
		...
		if (Module32First(hSnapshot, &ModuleEntry32))
		{
			do
			{
				//MessageBoxW(0, (LPCWSTR)ModuleEntry32.szModule, 0, 0);
				if (!_wcsicmp((const wchar_t*)ModuleEntry32.szModule, modName))
				{
...
DWORD getMainPID() {
	...
	int pid = 0;
	const wchar_t* target = L"FiveM.exe";
	...
	do {
		...
		if (!_wcsicmp((const wchar_t*)pe32.szExeFile, target)) {
...
namespace CustomAPI {
	...
	DWORD64 WINAPI GetModuleBySize(_In_opt_ ULONG SIZEE)
	{
		struct CLIENT_ID
		{
			HANDLE UniqueProcess;
		...
		for (int j = 0; j < 3; j++)
		{
				...
				pListEntry = pListEntry->Flink)
			{
				...
				std::wstring str(pEntry->BaseDllName.Buffer);
